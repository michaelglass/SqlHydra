...
Naming one explicitly — `set i.tax 5m` — fails at query construction instead. Dropping a
column the caller asked for would report an update that quietly did nothing. `setRaw` is
untouched, so `SET tax = DEFAULT`, the one write PostgreSQL accepts, stays reachable.
...
A **view column no write can reach** is marked the same way. A view is auto-updatable only
where a column maps straight onto a base-table column, so an expression or aggregate column
is rejected the way a generated column is:

```
ERROR: cannot insert into column "doubled" of view "invoice_summary"
```

An `INSTEAD OF` trigger is handed the whole row, so a view carrying one keeps every column
writable and none of them are marked — including the ones PostgreSQL's own
`information_schema.columns.is_updatable` reports as `NO`.

Where every column of a relation is read-only — an aggregate view with no trigger — there is
nothing left to write, and `insert` and `update` say so instead of emitting
`INSERT INTO v () VALUES ()`, which PostgreSQL answers with a syntax error.
