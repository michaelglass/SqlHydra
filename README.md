...
**Read-only columns.** A column PostgreSQL computes itself — `GENERATED ALWAYS AS (...) STORED`
...
The generator marks those columns `[<ReadOnlyColumn>]`, and SqlHydra.Query leaves them out
of every INSERT column list and UPDATE `SET` clause. A whole record round-trips without an
`excludeColumn` per generated column:

```fsharp
// INSERT INTO "sales"."invoice" ("seq", "price") VALUES (@p0, @p1)
insertTask db {
    into sales.invoice
    entity { id = 0; seq = 7; price = 10m; tax = 0m }   // id and tax are the database's
}
```

`set` is untouched: `set i.tax 5m` still fails at the database, as it does today.

Reads are unaffected: a generated column comes back from `SELECT *` like any other.
`GENERATED BY DEFAULT AS IDENTITY` is writable, so it is not marked and the value you set
is still sent. Detection is PostgreSQL-only for now; the attribute is not.

**The write record.** A table with a read-only column also gets `{table}_write`: the same
record minus those columns, marked `IWriteOf<{table}>`. `writeEntity` and `writeEntities`
take it where `entity` and `entities` take the read record; the loop variable stays the read
record, so `where`, `getId`, `includeColumn` and `excludeColumn` select over it as before:
...
A table with no read-only column gets no write record. `onConflict ... doUpdate i.tax` still
selects over the read record, so a read-only column can reach that `SET`; the database
