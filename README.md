...
**Tables with database-owned columns.** A column PostgreSQL writes itself — `GENERATED ALWAYS AS (...) STORED`
or `GENERATED ALWAYS AS IDENTITY` — rejects any statement that names it:

```
ERROR: cannot insert a non-DEFAULT value into column "id"
```

`entity` names every field of the record, so writing such a table meant an `excludeColumn` per
generated column at every write site. Instead, every generated table record now lists the columns
it may write, and a table with database-owned columns gets a second record, `{table}_write`: the
read record minus those columns, marked `IWriteOf<{table}>`, which the read record projects onto:

```fsharp
type invoice = { id: int; seq: int; price: decimal; tax: decimal }
    interface IHasWrite<invoice_write> with
        member this.ToWrite() = { seq = this.seq; price = this.price }
    interface IWriteColumns with
        member this.WriteColumns =
            [ { WriteColumn.Name = "seq"; Value = box this.seq; ProviderDbType = None }
              { WriteColumn.Name = "price"; Value = box this.price; ProviderDbType = None } ]

and invoice_write = { seq: int; price: decimal }
    interface IWriteOf<invoice> with
        member this.WriteColumns = (* the same two *)
```

`WriteColumns` is what the query layer binds. For a generated record it never reflects over
fields, and `entity row` on the read record sends only the columns the database accepts. A record
without it (hand-written, or generated before this) is reflected as before. `writeEntity` and
`writeEntities` take the write record where `entity` and `entities` take the read record; the loop
variable stays the read record, so `where`, `getId`, `includeColumn` and `excludeColumn` select
over it as before:

```fsharp
// UPDATE "sales"."invoice" SET "seq" = @p0, "price" = @p1 WHERE ("sales"."invoice"."id" = @p2)
updateTask db {
    for i in sales.invoice do
    entity { row with price = 20m }   // row: a sales.invoice read back earlier; id and tax are not sent
    where (i.id = row.id)
}

// INSERT INTO "sales"."invoice" ("seq", "price") VALUES (@p0, @p1)
insertTask db {
    into sales.invoice
    writeEntity { seq = 7; price = 10m }   // sales.invoice_write: there is no id or tax to fill in
}

// The write record of a row read back, when a typed `…Write` operation needs it
updateTask db {
    for i in sales.invoice do
    writeEntity { toWrite row with price = 20m }
    where (i.id = row.id)
}

// INSERT INTO "sales"."invoice" ("seq", "price") VALUES (@p0, @p1) ON CONFLICT(seq) DO UPDATE SET price=EXCLUDED."price"
insertTask db {
    for i in sales.invoice do
    writeEntity { seq = 7; price = 10m }
    onConflict i.seq
    doUpdateWrite (fun w -> w.price)   // w: sales.invoice_write
}
```

What it adds is at compile time. A read-only column has no field on the write record, so
writing one cannot be expressed, and `IWriteOf` keeps the records from being swapped:

```
writeEntity readRow        // FS0001: The type 'sales.invoice' is not compatible with the type 'SqlHydra.IWriteOf<sales.invoice>'
writeEntity otherWriteRow  // FS0001: The type 'sales.other_write' is not compatible with the type 'SqlHydra.IWriteOf<sales.invoice>'
entity writeRow            // FS0001: This expression was expected to have type 'sales.invoice' but here has type 'sales.invoice_write'
doUpdateWrite (fun w -> w.tax)   // FS0039: The type 'invoice_write' does not define a field, constructor, or member named 'tax'.
```

`writeEntity` is what fixes the write record for the query. After `entity` nothing has, so
`doUpdateWrite (fun w -> w.price)` is FS0072, `The type of this expression could not be inferred
before accessing its members`; annotate the lambda, `(fun (w: sales.invoice_write) -> w.price)`, to
say which write record is meant.

`set i.tax 5m` still reaches the database and is rejected there. `GENERATED BY DEFAULT AS IDENTITY`
is writable, so it stays on the write record. A table with no read-only column gets no write record, nor does one
where every column is read-only, since a record cannot be empty. The upsert keywords that name
update columns — `doUpdate`, `doUpdateCoalesce`, `onConflictDoUpdate` and `onConflictDoUpdateCoalesce`
on PostgreSQL, `onConflictDoUpdate` on SQLite, `insertOrUpdateOnUnique` on SQL Server — still select
them over the read record, so `doUpdate i.tax` still reaches the database; each has a `…Write` twin
that takes them as a lambda over the write record instead. Detection is PostgreSQL-only for now; the
write record, `writeEntity` and the `…Write` twins are not.
