...
`Column.IsReadOnly` says the database owns the value. The field is emitted carrying
`[<ReadOnlyColumn>]`, and `SqlHydra.Query` then keeps it out of every `INSERT` column list and
`UPDATE SET` clause -- so the record can hold the field, as a record must, without whatever it
holds ever reaching the statement. Reads are untouched.
