...
let visitWhere<'T> (tables: TableMapping seq) (filter: Expression<Func<'T, bool>>) (qualifyColumn: string -> MemberInfo -> string) : WhereClause =
    ...
    let rec visit (nexp: NormalizedExpression) : WhereClause =
        ...
        | NBinaryCompare(left, op, right) ->
            ...
            // Fall-throughs for a column compared to anything not matched above: the other
            // side is a captured value, so compile-and-eval it. These MUST stay below the
            // SQL-function arms. A SqlHydra function's body is a stub returning
            // `Unchecked.defaultof<_>`, so evaluating one yields null and the comparison
            // would silently collapse to `col IS NULL`.
            | NColumn (p, _), _ ->
                let alias = visitAlias p.Expression
                let fqCol = qualifyColumn alias p.Member
