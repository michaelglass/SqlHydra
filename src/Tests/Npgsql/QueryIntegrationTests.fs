...
// Read-only columns end to end, against a table PostgreSQL will not let you write to.

module ReadOnlyColumnFixture =
    module sales =
        /// Mirrors the DDL below, marked the way the schema provider marks it. `seq` is
        /// GENERATED BY DEFAULT, which is writable, so it carries no attribute.
        [<CLIMutable>]
        type sqlhydra_readonly =
    ...
    let invoices = table<sales.sqlhydra_readonly>

    let ddl =
        """
        DROP TABLE IF EXISTS sales.sqlhydra_readonly;
        CREATE TABLE sales.sqlhydra_readonly (
    ...
    /// The database owns `id` and `tax`, so the values here are never sent.
    let row : sales.sqlhydra_readonly =
        { id = 999
          seq = 7
          price = 10m
          tax = 999m }
...
[<Test>]
let ``read-only columns: an entity insert into a table with generated columns needs no excludeColumn``() = task {
    use! ctx = db.OpenContextAsync()
    do! ReadOnlyColumnFixture.exec ctx ReadOnlyColumnFixture.ddl

    let! inserted =
        insertTask ctx {
            into ReadOnlyColumnFixture.invoices
    ...
    let row = rows |> Seq.exactlyOne
    // The database assigned these; the 999s never left the client.
    row.id =! 1
    row.tax =! 1.0m
    // GENERATED BY DEFAULT is writable, so this one is still ours.
    row.seq =! 7
...
[<Test>]
let ``read-only columns: an entity update on the same table writes only the writable ones``() = task {
    use! ctx = db.OpenContextAsync()
    do! ReadOnlyColumnFixture.exec ctx ReadOnlyColumnFixture.ddl
    ...
    // `entity` builds the SET clause from every field. Naming id or tax fails the statement.
    let! updated =
        updateTask ctx {
            for i in ReadOnlyColumnFixture.invoices do
            entity { ReadOnlyColumnFixture.row with price = 20m }
            where (i.seq = 7)
        }
    updated =! 1
    ...
    let row = rows |> Seq.exactlyOne
    row.price =! 20m
    // Recomputed by the database from the new price.
    row.tax =! 2.0m
