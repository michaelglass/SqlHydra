...
module ReadOnlyColumnFixture =
    module sales =
        /// Mirrors the DDL below, marked the way the schema provider marks it.
        [<CLIMutable>]
...
let ``read-only columns: an entity insert into a table with generated columns needs no excludeColumn``() = task {
    ...
    let! row = ReadOnlyColumnFixture.readOne ctx
    row.id =! 1
    row.tax =! 1.0m
...
let ``read-only columns: getId returns the generated id, though the field list carries the other read-only columns``() = task {
    // `getId` routes through `excludeColumn`, so the field list carries `tax`, named by nobody.
    use! ctx = db.OpenContextAsync()
    do! ReadOnlyColumnFixture.exec ctx ReadOnlyColumnFixture.ddl
