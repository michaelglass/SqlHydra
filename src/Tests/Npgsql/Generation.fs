    ...
    try
        ...
        let readOnly =
            ...
            |> List.collect (fun tbl ->
                ...
                |> List.map (fun col -> tbl.Schema, tbl.Name, col.Name))
            |> Set.ofList
        ...
        readOnly =! set [ "sales", "sqlhydra_readonly_probe", "id"; "sales", "sqlhydra_readonly_probe", "tax" ]
    finally
        execute dropProbes
...
let private generated filters =
    (NpgsqlSchemaProvider.getSchema(filterCfg filters, false, [])).Tables

let private generatedPaths filters =
    generated filters |> List.map (fun tbl -> $"{tbl.Schema}/{tbl.Name}")

let private stateProvinceMatView filters =
...
let ``Column filters apply to materialized views too``() =
    let matView =
        stateProvinceMatView { Filters.Empty with Includes = [ "*"; "*.*" ]; Excludes = [ "person/vstateprovincecountryregion.territoryid" ] }
...
let ``A materialized view's columns are typed like a table's``() =
    ...
    let matView = stateProvinceMatView { Filters.Empty with Includes = [ "*"; "*.*" ] }
